-- ====================================================================
-- FOX Script V1 | FIXED ORANGE MOD MENU PREMIUM EDITION
-- FIXED CRASHES, BUTTON CLICKS, AND ALL UI ALIGNMENT ERRORS
-- ====================================================================
...
screenGui.Name = "FoxScriptV1_OrangeFixed"
screenGui.Parent = player:WaitForChild("PlayerGui")
screenGui.ResetOnSpawn = false

-- ========== زر المود منيو الدائري الطافي بصورتك الخاصه (تم إصلاح الضغط والسحب) ==========
...
modMenuCircle.Image = "rbxassetid://6073173223" 
...
-- ========== القائمة الرئيسية (FOX Script V1) باللون البرتقالي ==========
...
contentFrame.CanvasSize = UDim2.new(0, 0, 0, 600) -- مساحة طولية محسنة لمنع المشاكل
...
-- ========== زر ديسكورد الدائري المتحرك من جهة اليمين تماماً ==========
...
discordFloatingBtn.MouseButton1Click:Connect(function()
    if setclipboard then
        ...
        hint.Text = "FOX Script: Discord Link Copied Successfully!"
...
-- ========== نافذة الكشف باللون البرتقالي (Sub-Panel Source) ==========
...
-- ========== نافذة الايمبوت باللون البرتقالي (Aimbot Panel) ==========
...
-- دائرة الـ FOV للايمبوت (برتقالية)
...
-- ========== مقبض دائري للتحكم بالـ FOV ==========
local fovSliderFrame = Instance.new("Frame")
fovSliderFrame.Size = UDim2.new(0.9, 0, 0, 45)
...
-- ========== الأنظمة البرمجية ومحرك الكشف والهاك ==========
local esps = {}
local chamsObjects = {}
...
-- ========== دالة إنشاء الأزرار بنمط ومفاتيح iOS السلسة ==========
...
-- ========== إضافة أزرار ميزات الهكر والوظائف ==========
...
-- ========== [تعديل برمجى] دالة فتح وإغلاق الواجهة بدون أي لاج أو تعارض مع السحب ==========
...
-- نظام سحب اللوحات الاحترافي الآمن
...
-- دالة سحب آمنة ومخصصة للزر الدائري لا تعترض مع نقرات الـ Click
...
modMenuCircle.InputChanged:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then
        if circleDragStart then
            ...
            if delta.Magnitude > 7 then -- للتفريق بين السحب والضغط العادي
                circleDragging = true
...
-- المحرك المستمر الفوري الخالي من الـ Lag
...
print("FOX Script V1 Fixed Orange Hub Loaded Successfully!")
